...
def add_attendee(event_id: int, name: str):
...
def get_all_events() -> List[Event]:
    session = db_session.create_session()
    events = session.query(Event).order_by(Event.event_datetime.desc()).all()
    return events
...
def find_event_by_slug(event_slug: str):
    session = db_session.create_session()
...
def create_event(eventname: str, location: str, event_datetime, description: str, user_id: int) -> Optional[Event]:
    ...
    event.user_id = user_id   # 👈 link event to the user
